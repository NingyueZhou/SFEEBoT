...
def run_single_sf_cluster(wb_sf_dfs):
    ...
    for tissue in tissue_2_wb_tissue_df:
        ...
        for sf_ensembl in sf_ensembl_ids:
            ...
            # ------------------------------------
            #print(f'------------------- lr M (CF + TPM + PSI) : {tissue} : {sf_ensembl_2_name[sf_ensembl]} ({sf_ensembl})-----------------------')
    ...
    #joblib.dump(tissue_2_result_df, '/nfs/home/students/ge52qoj/SFEEBoT/output/result_lr_cluster.sav')
    return tissue_2_result_df
...
def analyse_result_cluster(tissue_2_result_df):
    ...
    for tissue in tissue_2_result_df:
        ...
        if result_df.empty == True:
            ...
            plt.figure(figsize=(14, 8))
            # print(result_df['M1_R2_std'].size)
            sns.barplot(x='sf_name', y='M1_mean_R2_score', data=result_df, palette='Blues', capsize=0, ci=None, yerr=result_df['M1_95_ci'])  # ci='sd'
            ...
            # ------------------------------------
            plt.figure(figsize=(14, 8))
            sns.barplot(x='sf_name', y='M2_mean_R2_score', data=result_df, palette='Blues', capsize=0, ci=None, yerr=result_df['M2_95_ci'])
            plt.axhline(y=M2_performance_threshold, c='black', lw=1, linestyle='dashed')
            plt.xticks(rotation=90)
            plt.ylabel('R2 score')
            ...
            if result_df[result_df['tissue_specific'] == 1].shape[0] > 0:
                ...
                plt.figure(figsize=(14, 8))
                sns.barplot(x='sf_name', y='M1_mean_R2_score', data=result_df[result_df['tissue_specific'] == 1], palette='Greens', capsize=0, ci=None, yerr=result_df[result_df['tissue_specific'] == 1]['M1_95_ci'])
                #plt.axhline(y=M2_performance_threshold, c='black', lw=1, linestyle='dashed')
                plt.xticks(rotation=90)
                plt.ylabel('R2 score')
            ...
            if result_df.loc[result_df['M2_mean_R2_score'] >= M2_performance_threshold].shape[0] == 0:
                ...
                enr = gp.enrichr(gene_list=gene_ids_df['Gene name'].values.tolist(),
                                 gene_sets='KEGG_2016',
                                 organism='Human',
                                 description= 'lr' + '_' + tissue,
                                 outdir='/nfs/home/students/ge52qoj/SFEEBoT/output/enrichr_kegg',
                                 cutoff=cutoff,
                                 no_plot=True)
                barplot(enr.res2d, title='KEGG_2016', cutoff=cutoff, ofname=f'/nfs/home/students/ge52qoj/SFEEBoT/output/enrichr_kegg/lr_cluster_{tissue}_kegg_bar.png')
                ...
                for sf_name in result_df['sf_name']:
                    if len(result_df.loc[result_df['sf_name'] == sf_name, 'most_important_feature_genes'].values.tolist()[0]) > 0:
                        ...
                        enr = gp.enrichr(gene_list=gene_ids_df['Gene name'].values.tolist(),
                                         gene_sets='KEGG_2016', #['KEGG_2016', 'KEGG_2013'],
                                         organism='Human',
                                         description= 'lr' + '_'+ tissue + '_' + sf_name,
                                         outdir='/nfs/home/students/ge52qoj/SFEEBoT/output/enrichr_kegg',
                                         cutoff=0.5,  # test dataset, use lower value from range(0,1)
                                         no_plot = True)
                        #print(enr.results.head(3))
                        if '/' in sf_name:
                            barplot(enr.res2d, title='KEGG_2016', cutoff=0.5, ofname='/nfs/home/students/ge52qoj/SFEEBoT/output/enrichr_kegg/lr_cluster_{}_{}_kegg_bar.png'.format(tissue, sf_name.split('/')[0]))
                        else:
                            barplot(enr.res2d, title='KEGG_2016', cutoff=0.5, ofname=f'/nfs/home/students/ge52qoj/SFEEBoT/output/enrichr_kegg/lr_cluster_{tissue}_{sf_name}_kegg_bar.png')
    ...
    # ------------------------------------
    plt.figure(figsize=(10, 8))
    sns.violinplot(x='tissue', y='M2_mean_R2_score', data=big_result_df)
    plt.ylabel('R2 score')
    plt.xlabel('')
    plt.xticks(rotation=90)
    ...
    if big_result_df[big_result_df['tissue_specific'] == 1].shape[0] > 0:
        ...
        # ------------------------------------
        plt.figure(figsize=(10, 8))
        sns.violinplot(x='tissue', y='M2_mean_R2_score', data=big_result_df_tissue_speci_sf)
        plt.ylabel('R2 score')
        plt.xlabel('')
        plt.xticks(rotation=90)
    ...
    if big_result_df[big_result_df['M2_significant_M0'] == 1].shape[0] > 0:
        ...
        # ------------------------------------
        plt.figure(figsize=(10, 8))
        sns.violinplot(x='tissue', y='M2_mean_R2_score', data=big_result_df_M2_to_M0_signif_sf)
        plt.ylabel('R2 score')
        plt.xlabel('')
        plt.xticks(rotation=90)
