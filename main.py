...
def main():
    ...
    if args.process_data_flag.upper() == 'TRUE':
        if args.dimension_reduct_method.lower() == 'pca':
            dfs = reader.read_all(tpm=args.tpm, att=args.att, pheno=args.pheno, ginf=args.ginf, sf=args.sf, psi=args.psi, drm=args.dimension_reduct_method.lower())  #srrgtex=args.srrgtex,
    ...
    if model == 'en':
        ...
        if args.compute_result_flag.upper() == 'TRUE':
            if args.dimension_reduct_method.lower() == 'pca':
                ...
                tissue_2_result_df = model_en.run_single_sf_cluster(wb_sf_dfs)
                joblib.dump(tissue_2_result_df, result_obj_file_name_cluster)
                model_en.analyse_result_cluster(tissue_2_result_df)
        elif args.compute_result_flag.upper() == 'FALSE':
            if args.dimension_reduct_method.lower() == 'pca':
                tissue_2_result_df = joblib.load(result_obj_file_name_pca)
                model_en.analyse_result_pca(tissue_2_result_df)
            elif args.dimension_reduct_method.lower() == 'cluster':
                tissue_2_result_df = joblib.load(result_obj_file_name_cluster)
                model_en.analyse_result_cluster(tissue_2_result_df)
    if model == 'lr':
        ...
        if args.compute_result_flag.upper() == 'TRUE':
            if args.dimension_reduct_method.lower() == 'pca':
                ...
                tissue_2_result_df = model_lr.run_single_sf_cluster(wb_sf_dfs)
                joblib.dump(tissue_2_result_df, result_obj_file_name_cluster)
                model_lr.analyse_result_cluster(tissue_2_result_df)
        elif args.compute_result_flag.upper() == 'FALSE':
            if args.dimension_reduct_method.lower() == 'pca':
                tissue_2_result_df = joblib.load(result_obj_file_name_pca)
                model_lr.analyse_result_pca(tissue_2_result_df)
    ...
    if model == 'rf':
        ...
        if args.compute_result_flag.upper() == 'TRUE':
            if args.dimension_reduct_method.lower() == 'pca':
                ...
                tissue_2_result_df = model_rf.run_single_sf_cluster(wb_sf_dfs)
                joblib.dump(tissue_2_result_df, result_obj_file_name_cluster)
                model_rf.analyse_result_cluster(tissue_2_result_df)
        elif args.compute_result_flag.upper() == 'FALSE':
            if args.dimension_reduct_method.lower() == 'pca':
                tissue_2_result_df = joblib.load(result_obj_file_name_pca)
                model_rf.analyse_result_pca(tissue_2_result_df)
